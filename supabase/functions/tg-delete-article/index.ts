...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (article.author_id !== profile.id) {
      return new Response(JSON.stringify({ error: 'You can only delete your own articles' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(`[tg-delete-article] Article ${articleId} deleted by user ${tgUser.id}`);

    return new Response(JSON.stringify({ success: true }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (e) {
    console.error('tg-delete-article error:', e);
